...
public class Comments extends Connect
{
	...
	{
		String SQL = "INSERT INTO `comments` (`comment_notification_id`, `comment_name`, `comment_email`, `comment_description`) VALUES (?, ?, ?, ?);";
		int record=0; 
		String error = "";
		...
		try
		{
			...
			pstmt.setString(1,(String) ticketData.get("comment_notification_id"));
    ...
    public ArrayList getAllComments(String notification_id)
	{
		String SQL = "SELECT * FROM `comments` WHERE comment_notification_id = "+notification_id+" ORDER BY comment_id ASC";
		int count=0;
