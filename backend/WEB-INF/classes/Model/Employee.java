...
public class Employee extends Connect
{
	...
	String SQL = "INSERT INTO employee (employee_sal, employee_first_name, employee_middle_name, employee_last_name, employee_gender, employee_address, employee_village, employee_state, employee_country, employee_landline, employee_mobile, employee_email, employee_status, employee_deparment, employee_dob, employee_nationalty, employee_role) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,  ?, ?, ?, ?, ?, ?, ?);";
	...
	try
	{
		...
		pstmt.setString(17,(String) employeeData.get("employee_role"));
	...
	{
		String SQL = "UPDATE employee SET employee_sal = ?, employee_first_name = ?, employee_middle_name = ?, employee_last_name = ?, employee_gender = ?, employee_address = ?, employee_village = ?, employee_state = ?, employee_country = ?, employee_landline = ?, employee_mobile = ?, employee_email = ?, employee_status = ?, employee_deparment = ?, employee_dob = ?, employee_nationalty = ?, employee_role = ? WHERE employee_id = ?";
		String error = "";
		...
		try
		{
			...
			pstmt.setString(16,(String) employeeData.get("employee_nationalty"));
			pstmt.setString(17,(String) employeeData.get("employee_role"));
			pstmt.setString(18,(String) employeeData.get("employee_id"));
			record = pstmt.executeUpdate();
			pstmt.close();
			connection.close();
		}
    ...
    public ArrayList getAllEmployee(String managerID)
	{
        ...
        try
		{
			...
			{
				SQL = "SELECT * FROM employee WHERE employee_role = "+managerID;
			}
            statement = connection.createStatement();
            rs = statement.executeQuery(SQL);
            while(rs.next())
