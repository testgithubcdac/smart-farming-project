...
<%
	...
	if((request.getParameter("act")).equals("chk_login"))
	{
		if(login_obj.checkLogin(request.getParameter("login_user"),request.getParameter("login_password")))
		{
			...
			out.print(employeeValues.get("employee_status"));
			if(Integer.parseInt(employeeValues.get("employee_status").toString()) == 2) {
				response.sendRedirect("../login.jsp?msg=You account has been blocked. Kindly contact to admin !!!");
			} else {
